...
async def confirm_with_game(uid: str):
    ...
    async with get_monitor():
        ...
        if specs:
            ...
            raise Exception(f"unknown field uuid {uid}")


@app.get("/info/{uid}")
async def info_about_game(uid: str):
    """Get information about a game given a field UUID"""
    async with get_monitor():
        specs = roborest.status_keeper.get_reservation(uid)
        if specs:
            player_id = specs["player_id"]
            ...
            player_status = roborest.status_keeper.get_player_status(game_id, player_id)
            return {
                "status": status,
                "player_id": player_id,
                "game_id": game_id,
